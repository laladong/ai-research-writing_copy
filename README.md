...
4. 输出格式：
   - Part 1：只输出缩减后的文字本身。
     * 保持数学公式原样（保留 `$` 符号）。
   - Part 2 [Modification Log]：使用中文简要说明你调整了哪些地方（例如：删除了冗余词 "XXX"，合并了 "YYY" 从句）。
   - 除以上2部分外，不要输出任何多余的对话。
...
# Task
请对我提供的片段进行“去 AI 化”重写，使其语言风格接近人类母语研究者。
...
5. 输出格式：
   - Part 1 ：输出重写后的文字（如果原文已足够好，则输出原文）。
     * 必须对特殊字符进行转义（例如：`%`、`_`、`&`）。
     * 保持数学公式原样（保留 `$` 符号）。
...
# Task
请将我提供的【中文描述】转化为符合顶级会议规范的【图标题】。

# Constraints

1. 写作风格：
   - 极简原则：去除冗余开头，直接描述图表内容。
